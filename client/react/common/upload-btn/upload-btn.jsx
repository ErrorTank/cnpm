  ...
  handleUpload = e => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file && file.type.indexOf("image") > -1) {
        promise.push(getBase64(file).then((base64File) => {
          if (!imagesPreview.find(each => each.file.name === base64File.file.name))
            return base64File;
          else {
            onError(<span>Bỏ qua file <span style={{color: "#189eff"}}>{base64File.file.name}</span> do đã tồn tại.</span>);
            return null;
          }
        }));
