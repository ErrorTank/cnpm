...
const getProducts = (
  ...
  return Promise.resolve()
    ...
    .then(categories => {
      let findFunc = categoryID
        ? createFindCategories(categories)
        : () => Promise.resolve();
      return findFunc(categoryID)
        .then(data => {
          ...
          if (provider) {
            pipelines.push({
              $match: {
                "provider.owner._id": mongoose.Types.ObjectId(provider)
              }
            });
          }
        ...
        .then(data => {
          ...
          let currentCateIsParent = categoryID
            ? !!allCates.find(each => each.parent === categoryID)
            : null;
          let currentCate = categoryID
            ? allCates.find(each => each._id.toString() === categoryID)
            : null;
